...
router.post('/import-users', uploadExcel.single('file'), async function (req, res, next) {
    try {
        ...
        let results = await importController.ImportUsersFromExcel(filePath);
        ...
        res.send({
            success: true,
            message: `Import thanh cong ${results.success.length} users, that bai ${results.failed.length} users`,
            data: results
        });
